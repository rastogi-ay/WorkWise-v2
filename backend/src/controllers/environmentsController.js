...
async function list(req, res) {
  try {
    const environments = await environmentsService.listEnvironments(req.stiggCustomerId);
...
async function add(req, res) {
  ...
  try {
    const environments = await environmentsService.addEnvironment(req.stiggCustomerId, name, {
      clientApiKey,
      serverApiKey,
    });
